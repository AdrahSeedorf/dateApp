...
import Unseal, { SealBreak } from "@/components/letters/Unseal";
...
export default async function LetterPage({ params, searchParams }: Props) {
  ...
  // Set by the redirect after breaking the seal, so the ceremony plays once
  // rather than every time the letter is re-read.
  const justOpened = just === "opened";
  ...
  return (
    <Screen className="mx-auto max-w-2xl">
      ...
      <div className="mb-space-lg flex items-start gap-space-md">
        <SealBreak justOpened={justOpened}>
          <span
            aria-hidden
